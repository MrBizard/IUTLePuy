...
char ChoisirMenu()
{
  ...
  std::cout << "\n\n\n"
    ...
    "6 : Nombres premiers\n"
    "7 : Nombres parfaits\n"
    "8 : Nombres premiers et parfaits\n"
    "q : Quitter\n"
    "\n"
    "Votre choix : ";
  std::cin >> choix;
 ...
 * \return Un tableau d'entier contenant tous les nombres parfaits inférieurs à la limite max.
 */
void AfficheNombrePremierEtParfait(unsigned int max)
{
  std::vector<unsigned int> nombres;

  nombres = TrouverNombrePremier(max);
  std::cout << "Les nombres premiers inférieurs à " << max << " sont : ";
  for (auto p : nombres)
    std::cout << p << ' ';

  std::cout << '\n';

  nombres = TrouverNombreParfait(max);
  std::cout << "Les nombres parfaits inférieurs à " << max << " sont : ";
  for (auto p : nombres)
    std::cout << p << ' ';
...
int main()
{
  ...
  while (choix != 'q' && choix != 'Q')
  {
    ...
    {
      unsigned int limite = SaisirEntierPositif("Veuillez entrer la limite de la recherche");
      AfficheNombrePremierEtParfait(limite);
    }
      break;
    case 'q':
    case 'Q':
